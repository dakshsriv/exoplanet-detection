  ...
  for predictions in estimator.predict(input_fn):
    ...
    with open('res.txt', 'w') as res:
      if predictions[0] >= 0.5:
        res.write(f'is an exoplanet. The original prediction is {predictions[0]}.')
      else:
        res.write(f'is not an exoplanet. The original prediction is {predictions[0]}.')
    with open('res.txt', 'r') as res:
      res = res.read()
      print(f'res is {res}')
#      for x, y in zip(time, flux):
#        if y == min(flux):
#          print(x, y)
